...
说明:
  - 使用 GPTQModel 将 HuggingFace 的 HY-MT1.5-1.8B 量化为 GPTQ Int4
...
try:
    ...
    def _load_weight_keys(model_dir: str) -> List[str]:
    ...
    def _detect_root_and_layers(keys: List[str]) -> Tuple[List[str], str, str]:
        candidates = [
        ...
        layer_container_candidates = ["layers", "h", "blocks"]
        for root, _ in candidates:
            for container in layer_container_candidates:
                prefix = f"{root}.{container}.0."
                if any(k.startswith(prefix) for k in keys):
                    return root.split("."), root, container
        for container in layer_container_candidates:
            prefix = f"{container}.0."
            if any(k.startswith(prefix) for k in keys):
                return [], "", container
        return ["model"], "model", "layers"


    def _detect_module_name(keys: List[str], prefix: str, suffix: str) -> str | None:
        for key in keys:
            ...
            if key.endswith(suffix):
                ...
                if len(parts) > len(suffix_parts):
                    return parts[-(len(suffix_parts) + 1)]
        return None


    def _detect_norm_module(keys: List[str], prefix: str, candidates: List[str]) -> str | None:
        for name in candidates:
            suffix = f"{name}.weight"
            if any(k.startswith(prefix) and k.endswith(suffix) for k in keys):
                return name
        return None


    def _detect_proj_set(keys: List[str], prefix: str, module_name: str, candidates: List[Tuple[str, str, str, str]]):
        for q, k, v, o in candidates:
            if all(
                any(
                    kname.startswith(prefix)
                    and kname.endswith(f"{module_name}.{proj}.weight")
                    for kname in keys
                )
                for proj in (q, k, v, o)
            ):
    ...
    def _detect_mlp_proj_set(keys: List[str], prefix: str, module_name: str, candidates: List[Tuple[str, str, str]]):
        for gate, up, down in candidates:
            if all(
                any(
                    kname.startswith(prefix)
                    and kname.endswith(f"{module_name}.{proj}.weight")
    ...
    def _detect_lm_head_and_norm(keys: List[str]) -> Tuple[str, str | None]:
        lm_head = "lm_head"
        pre_norm = None
        for key in keys:
            if key.endswith("lm_head.weight"):
                lm_head = key[: -len(".weight")]
                break
        if "model.norm.weight" in keys:
            pre_norm = "model.norm"
        elif "transformer.norm.weight" in keys:
            pre_norm = "transformer.norm"
        elif "language_model.model.norm.weight" in keys:
            pre_norm = "language_model.model.norm"
        elif "norm.weight" in keys:
            pre_norm = "norm"
        return lm_head, pre_norm


    def _infer_hunyuan_module_tree(model_dir: str):
        keys = _load_weight_keys(model_dir)
        root_parts, root_prefix, layer_container = _detect_root_and_layers(keys)
        layer_prefix = f"{root_prefix}.{layer_container}.0." if root_prefix else f"{layer_container}.0."

        attn_module = _detect_module_name(keys, layer_prefix, "q_proj.weight")
        if not attn_module:
            attn_module = _detect_module_name(keys, layer_prefix, "wq.weight")
        attn_module = attn_module or "self_attn"

        mlp_module = _detect_module_name(keys, layer_prefix, "gate_proj.weight")
        if not mlp_module:
            mlp_module = _detect_module_name(keys, layer_prefix, "w1.weight")
        mlp_module = mlp_module or "mlp"

        input_norm = _detect_norm_module(
            keys,
            layer_prefix,
            ["input_layernorm", "ln_1", "attention_norm", "attn_norm", "pre_attention_layernorm", "input_norm"],
        ) or "input_layernorm"

        post_norm = _detect_norm_module(
            keys,
            layer_prefix,
            ["post_attention_layernorm", "ln_2", "ffn_norm", "post_attention_norm", "post_norm", "mlp_norm"],
        ) or "post_attention_layernorm"

        attn_proj_set = _detect_proj_set(
            keys,
            layer_prefix,
            attn_module,
            [
        ...
        if attn_proj_set is None:
            attn_proj_set = ("q_proj", "k_proj", "v_proj", "o_proj")

        mlp_proj_set = _detect_mlp_proj_set(
            keys,
            layer_prefix,
            mlp_module,
            [
        ...
        if mlp_proj_set is None:
            mlp_proj_set = ("gate_proj", "up_proj", "down_proj")

        lm_head, pre_norm = _detect_lm_head_and_norm(keys)

        module_tree = []
        module_tree.extend(root_parts)
        module_tree.extend([layer_container, "#"])
        module_tree.append(
            {
                "input_layernorm": (f"{input_norm}:!",),
                attn_module: (
                    f"{attn_proj_set[0]}:0",
                    f"{attn_proj_set[1]}:0",
                    f"{attn_proj_set[2]}:0",
                ...
                "post_attention_layernorm": (f"{post_norm}:!",),
                mlp_module: (
                    f"{mlp_proj_set[0]}:0",
                    f"{mlp_proj_set[1]}:0",
                    f"{mlp_proj_set[2]}:1",
        ...
        return module_tree, lm_head, pre_norm


    def ensure_hunyuan_v1_dense_supported(model_id_or_path: str):
        """Register HunYuan v1 Dense as a Llama-compatible QModel for gptqmodel."""
        try:
            from gptqmodel.models import auto as gptq_auto
        except Exception:
            return

        if "hunyuan_v1_dense" in getattr(gptq_auto, "MODEL_MAP", {}):
            return

        try:
            from gptqmodel.models.definitions.llama import LlamaQModel as BaseQModel
        except Exception:
            try:
                from gptqmodel.models.definitions.qwen2 import Qwen2QModel as BaseQModel  # type: ignore[import-not-found]
            except Exception:
                try:
                    from gptqmodel.models.definitions.qwen3 import Qwen3QModel as BaseQModel  # type: ignore[import-not-found]
                except Exception:
                    return
        ...
        try:
            ...
            if os.path.exists(config_path):
                with open(config_path, "r", encoding="utf-8") as f:
                    raw_cfg = json.load(f)
                if raw_cfg.get("model_type") != "hunyuan_v1_dense":
                    return
            model_dir = os.path.abspath(cfg._name_or_path)
        except Exception:
            model_dir = os.path.abspath(model_id_or_path)

        module_tree, lm_head, pre_norm = _infer_hunyuan_module_tree(model_dir)

        class HunyuanV1DenseQModel(BaseQModel):
            pass

        HunyuanV1DenseQModel.lm_head = lm_head
        HunyuanV1DenseQModel.pre_lm_head_norm_module = pre_norm or "model.norm"
        HunyuanV1DenseQModel.module_tree = module_tree

        gptq_auto.MODEL_MAP["hunyuan_v1_dense"] = HunyuanV1DenseQModel
        supported = getattr(gptq_auto, "SUPPORTED_MODELS", None)
        if isinstance(supported, list) and "hunyuan_v1_dense" not in supported:
            supported.append("hunyuan_v1_dense")
...
def main():
    ...
    ensure_hunyuan_v1_dense_supported(model_id)
